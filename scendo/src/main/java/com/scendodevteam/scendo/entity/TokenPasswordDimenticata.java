...
@NoArgsConstructor
@Entity
public class TokenPasswordDimenticata {
    ...
    nullable = false,
    foreignKey = @ForeignKey(name="FK_TOKENPASSWORDDIMENTICATA_UTENTE"))
    private Utente utente;

    public TokenPasswordDimenticata(Utente utente, String token) {
        super();
        this.token = token;
        this.utente = utente;
    ...
    public TokenPasswordDimenticata(String token) {
        super();
        this.token = token;
        this.dataScadenza = calcolaDataScadenza(EXPIRATION_TIME);
    }
